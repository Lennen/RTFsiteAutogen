...
$findme[8]   = 'III. Права Коммерческий агент имеет право';
$findme[9]   = 'IV. Ответственность Коммерческий агент несет ответственность';
$findme[10]   = 'Должностная инструкция разработана в соответствии';

$neuroFreqWords[0] = "первый";
$neuroFreqWords[1] = "хотеть";
$neuroFreqWords[2] = "надо";
$neuroFreqWords[3] = "должный";

$fr[0] = $result.count($neuroFreqWords[0]);
...
<html>
    ...
    <form action="rtf.php" method="post" enctype="multipart/form-data">
        <p>Загрузите RTF-файл:</p>
        <input type="file" name="uploadfile" id="rtf">
        <label for="rtf"> Выбрать</label>
        <p><input type="submit" value="Отправить"></p>
    </form>
	
	<?php for($i = 0; $i <= count($findme); $i++): ?>
	   <?=$findme[$i]?>: <?=$arr[$i+1]?>
	   <br/><br/>
    <?php endfor ?>
